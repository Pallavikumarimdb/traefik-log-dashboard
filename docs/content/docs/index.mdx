...
<Callout title="Latest Release: v2.4.0" type="info">
GeoIP simplified, performance optimizations, and reduced bundle size.
</Callout>
...
<Step>
### Dashboard
Next.js web UI with interactive charts, real-time updates, and beautiful visualizations including 3D globe and geographic maps. Includes automatic GeoIP lookup.
</Step>
...
- **GeoIP Simplified** - Geographic IP lookup now automatic in dashboard (no configuration needed)
- **Performance Optimizations** - 44% reduction in bundle size, React.memo on components
- **Agent Improvements** - Fixed O(n^2) operations, added slice pre-allocation
- **Documentation Updated** - Simplified deployment guides, removed outdated info
