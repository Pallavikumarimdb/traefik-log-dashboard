...
export default function ResponseTimeCard({ metrics }: Props) {
  ...
  return (
    <Card title="Response Time" icon={<Clock className="w-5 h-5 text-red-600" />}>
        ...
        {percentiles.map((percentile, idx) => (
          <div key={idx} className={`p-4 rounded-lg border ${getPerformanceBg(percentile.value)} transition-all hover:shadow-md`}>
            <div className="flex items-center justify-between mb-2">
              <div>
                <div className="text-sm font-semibold text-gray-900">{percentile.label}</div>
                <div className="text-xs text-gray-500">{percentile.description}</div>
              </div>
              ...
              <div className="h-2 w-full bg-gray-200 rounded-full overflow-hidden">
                <div
                  className={`h-full transition-all duration-500 ${
                    ...
                    percentile.value < 1000 ? 'bg-orange-500' : 'bg-red-500'
                  }`}
