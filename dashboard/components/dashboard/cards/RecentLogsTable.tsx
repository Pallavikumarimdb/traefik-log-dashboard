...
export default function RecentLogsTable({ logs }: Props) {
  ...
  const getStatusColor = (status: number): string => {
    if (status >= 200 && status < 300) return 'text-green-600 dark:text-green-400 font-semibold';
    if (status >= 300 && status < 400) return 'text-blue-600 dark:text-blue-400 font-semibold';
    if (status >= 400 && status < 500) return 'text-yellow-600 dark:text-yellow-400 font-semibold';
      ...
      icon={
        <div className="relative">
          <details className="relative">
            <summary className="flex items-center gap-1 cursor-pointer text-xs font-medium bg-primary/10 hover:bg-primary/20 px-3 py-1.5 rounded-md transition-colors list-none">
          ...
          <thead>
            <tr className="border-b-2 border-border">
              {visibleColumns.map(col => (
                <th
                  key={col.id}
                  className={`text-left py-3 px-3 text-xs font-bold uppercase tracking-wider bg-muted/50 ${('width' in col ? col.width : '') || ''}`}
                >
                  {col.header}
                </th>
              ))}
