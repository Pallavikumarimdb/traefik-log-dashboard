...
export default function UserAgentsCard({ userAgents }: Props) {
  if (!userAgents || userAgents.length === 0) {
    return (
      <Card title="User Agents" icon={<Monitor className="w-5 h-5 text-violet-600" />}>
        ...
        {topAgents.map((agent, idx) => (
          <div key={idx} className="space-y-2">
            <div className="flex items-center justify-between">
              <span className="text-sm font-medium truncate flex-1" title={agent.browser}>
                {agent.browser}
              </span>
              <div className="flex items-center gap-2 text-xs text-muted-foreground ml-2">
                <span className="font-medium">{formatNumber(agent.count)}</span>
                <span className="font-semibold text-violet-600">{agent.percentage.toFixed(1)}%</span>
              </div>
            </div>
